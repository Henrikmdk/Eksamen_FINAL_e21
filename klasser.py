# Parent klasse. Indeholder de attributter der er fælles for alle klasser samt metoder der håndterer disse.
class Materiale:
...
class Bog(Materiale):
    ...
    def toSQL(self, sqltype):
        ...
        if sqltype == 'opskriv_udlaan':
            return f"Update BOG set N_UDLAAN = {int(self.n_udlaan) + 1} where ID = {self.idnr}"
        if sqltype == 'opskriv_reservation':
            return f"Update BOG set RESERVEREDE = {int(self.reserverede) + 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_udlaan':
            return f"Update BOG set N_UDLAAN = {int(self.n_udlaan) + 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_reservation':
            return f"Update BOG set RESERVEREDE = {int(self.reserverede) - 1} where ID = {self.idnr}"
        if sqltype == 'slet':
            return f"DELETE FROM BOG WHERE ID={self.idnr}"
...
class Film(Materiale):
    ...
    def toSQL(self, sqltype):
        ...
        if sqltype == 'opskriv_udlaan':
            return f"Update FILM set N_UDLAAN = {int(self.n_udlaan) + 1} where ID = {self.idnr}"
        if sqltype == 'opskriv_reservation':
            return f"Update FILM set RESERVEREDE = {int(self.reserverede) + 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_udlaan':
            return f"Update FILM set N_UDLAAN = {int(self.n_udlaan) - 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_reservation':
            return f"Update FILM set RESERVEREDE = {int(self.reserverede) - 1} where ID = {self.idnr}"
        if sqltype == 'slet':
            return f"DELETE FROM FILM WHERE ID={self.idnr}"
# Child klasse. Arver fra Materiale de atributter og metoder der er fælles for alle klasser
class CD(Materiale):
    ...
    def toString(self):
        return f'ID:{self.idnr} (CD) {self.titel}({self.aarstal}), {self.kunstner}, ' \
               f'Tracks {self.tracks}({self.total_varighed} minutter), ' \
               f' Tilgængelighed {self.n_udlaan}/{self.antal}, Reserverede: {self.reserverede}'
    ...
    def toSQL(self, sqltype):
        ...
        if sqltype == 'opskriv_udlaan':
            return f"Update CD set N_UDLAAN = {int(self.n_udlaan) + 1} where ID = {self.idnr}"
        if sqltype == 'opskriv_reservation':
            return f"Update CD set RESERVEREDE = {int(self.reserverede) + 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_udlaan':
            return f"Update CD set N_UDLAAN = {int(self.n_udlaan) - 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_reservation':
            return f"Update CD set RESERVEREDE = {int(self.reserverede) - 1} where ID = {self.idnr}"
        if sqltype == 'slet':
            return f"DELETE FROM CD WHERE ID={self.idnr}"
...
class DRMLicens(Materiale):
    ...
    def toString(self):
        return f'ID:{self.idnr} ({self.formattype}) {self.titel}({self.aarstal}), {self.skaber}, ({self.tidsbegraensning}' \
               f' dage), Tilgængelighed {self.n_udlaan}/{self.antal}, Reserverede {self.reserverede}'
    ...
    def toSQL(self, sqltype):
        ...
        if sqltype == 'opskriv_udlaan':
            return f"Update DRMLICENS set N_UDLAAN = {int(self.n_udlaan) + 1} where ID = {self.idnr}"
        if sqltype == 'opskriv_reservation':
            return f"Update DRMLICENS set RESERVEREDE = {int(self.reserverede) + 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_udlaan':
            return f"Update DRMLICENS set N_UDLAAN = {int(self.n_udlaan) - 1} where ID = {self.idnr}"
        if sqltype == 'nedskriv_reservation':
            return f"Update DRMLICENS set RESERVEREDE = {int(self.reserverede) - 1} where ID = {self.idnr}"
        if sqltype == 'slet':
            return f"DELETE FROM DRMLICENS WHERE ID={self.idnr}"
